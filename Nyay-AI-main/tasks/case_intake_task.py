...
case_intake_task = Task(
    ...
    description=(
        "The user has submitted the following legal querry.\n\n"
        "{user_input}\n\n"
        "Your job is to interpret it , identify the core legal issue, classify the legal domain"
        "(eg. civil, criminal , labour), and return a structured JSON with : "
        "'case type',legal_domain' , 'summary' ,'relevant_entities' and 'jurisdiction'(if any)."
    ),
     expected_output=(
        "```json\n"
        "{\n"
